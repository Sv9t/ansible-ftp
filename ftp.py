...
citizen = namedtuple("Citizen", "IS_DIR IS_FILE NOT_EXISTS")
_FTPPathStatus = citizen(IS_DIR=1, IS_FILE=2, NOT_EXISTS=3)
...
def _ftp_compare_files(module, session, local_path, remote_path):
    ...
    md5 = hashlib.md5()
    session.retrbinary("RETR %s" % remote_path, md5.update)
...
def _ftp_backup_file(module, session, path):
    ...
    backup_path = "%s.%d.%s" % (path, os.getpid(), ext)
...
def _ftp_upload_file(module, session, src, dest):
    ...
    with open(src, "rb") as src_object:
        session.storbinary("STOR %s" % dest, src_object)
...
def _ftp_download_file(module, session, src, dest):
    ...
    with open(dest, "wb") as dest_object:
        session.retrbinary("RETR %s" % src, dest_object.write)
...
def _ftp_upload(module, session, src, dest, top_dir=True):
    ...
    if not os.access(src, os.R_OK):
        module.fail_json(msg="Source %s is not readable" % src)
    if not os.path.isfile(src) and not os.path.isdir(src):
        module.fail_json(
            msg="Source %s is not a regular file or a directory" % src
        )
    ...
    try:
        ...
        module.fail_json(
            msg="Unable to get status for destination %s: %s" % (
                dest, to_native(ex)
            )
        )
    ...
    dest_is_dir |= dest_status == 1 or os.path.isdir(src)

    # if os.path.isdir(src) and dest_status == _FTPPathStatus.IS_FILE:
    if os.path.isdir(src) and dest_status == 2:
        module.fail_json(
            msg="Destination %s is a file whereas source %s is a "
            "directory" % (dest, src)
        )
    ...
    if not dest_exists:
        ...
        try:
            ...
            module.fail_json(
                msg="Unable to create destination directory %s: %s" % (
                    dest, to_native(ex)
                )
            )
    ...
    if results["changed"]:
        try:
            ...
            module.fail_json(
                msg="Unable to backup destination %s: %s" % (
                    dest, to_native(ex)
                )
            )
        ...
        try:
            ...
            module.fail_json(
                msg="Unable to upload source %s to destination %s: %s" % (
...
def _ftp_download(module, session, src, dest, top_dir=True):
    ...
    try:
        ...
        module.fail_json(
            msg="Unable to get status for source %s: %s" % (
                src, to_native(ex)
            )
        )
    ...
    dest_is_dir |= src_status == 1 or os.path.isdir(dest)
    if dest_exists:
        if not os.access(dest, os.W_OK):
            module.fail_json(msg="Destination %s is not writable" % dest)
        if not dest_is_dir and not os.path.isfile(dest):
            module.fail_json(
                msg="Destination %s is not a regular file or a "
                "directory" % dest
            )

    # if dest_exists and not dest_is_dir and src_status == _FTPPathStatus.IS_DIR:
    if dest_exists and not dest_is_dir and src_status == 1:
        module.fail_json(
            msg="Source %s is a directory whereas destination %s is not a "
            "directory" % (dest, src)
        )
    ...
    if results["changed"]:
        ...
        if tmp_dest:
            ...
            if not os.path.isdir(tmp_dest):
                if os.path.exists(tmp_dest):
                    module.fail_json(
                        msg="%s is a file but should be a directory" % 
                            tmp_dest
                    )
                else:
                    module.fail_json(
                        msg="%s directory does not exist" % tmp_dest
                    )
        ...
        try:
            ...
            module.fail_json(
                msg="Unable to download source %s to destination %s: "
                "%s" % (src, dest, to_native(ex))
            )
...
def main():
    ...
    module = AnsibleModule(
        argument_spec={
            ...
            "backup": {"type": "bool", "choices": [False, True], "default": False},
            "timeout": {"type": "int"},
    ...
    with contextlib.closing(FTP()) as session:
        try:
            ...
            module.fail_json(
                msg="Unable to connect to FTP server: %s" % to_native(ex)
            )
